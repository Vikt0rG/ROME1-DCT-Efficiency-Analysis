...
Cluster::Cluster(Hit* first_hit, EtaSide side, int layer)
    : _layer(layer)
    , _eta_side(side) {
    _cluster_hits.push_back(first_hit);
...
bool Cluster::addHit(Hit* hit) {
    ...
    for (size_t idx = 0; idx < _cluster_hits.size(); idx++) {

        // Check if hit is on the same or neighboring strip
        int strip_diff = abs(hit->getStrip() - _cluster_hits[idx]->getStrip());
        if (strip_diff > MAX_STRIP_DISTANCE) continue;

        // Check if hit is within time window (use correct eta side)
        int time_diff;
        if (_eta_side == ETA1) {
            time_diff = abs(hit->getTimeEta1() - _cluster_hits[idx]->getTimeEta1());
        } else {
            time_diff = abs(hit->getTimeEta2() - _cluster_hits[idx]->getTimeEta2());
        }
